...
class Linear(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        """
        return input @ self.weight.T + (self.bias if self.bias is not None else np.zeros((self.out_features,)))
...
class BatchNormalization(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        :return: array of shape (batch_size, num_features)
        """
        dx = grad_output * self.weight if self.affine else grad_output
        if self.training:
            din = np.sum(dx * self.input_mean, axis=0)
            dmu1 = dx * self.inv_sqrt_var
            dsqrt = -din / self.sqrt_var ** 2
            dvar = 0.5 * dsqrt * self.inv_sqrt_var
            dl_ds = dvar / input.shape[0]
            dmu2 = 2 * self.input_mean * dl_ds
            dx1 = dmu1 + dmu2
            dmu = -1 * np.sum(dx1, axis=0) / input.shape[0]
            return dx1 + np.ones(input.shape) * dmu
...
class Sequential(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        for module in reversed(self.modules):
            grad_output = module.backward(self.inputs[module], grad_output)
        return grad_output
