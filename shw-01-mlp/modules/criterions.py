...
class MSELoss(Criterion):
    ...
    def compute_output(self, input: np.ndarray, target: np.ndarray) -> float:
        ...
        return np.float64(np.mean((input - target) ** 2))
...
class CrossEntropyLoss(Criterion):
    ...
    def compute_output(self, input: np.ndarray, target: np.ndarray) -> float:
        ...
        return -np.mean(log_probs[np.arange(len(target)), target])
    ...
    def compute_grad_input(self, input: np.ndarray, target: np.ndarray) -> np.ndarray:
        ...
        """
        probs = np.zeros(input.shape)
        probs[np.arange(len(target)), target] = 1
        return self.log_softmax.compute_grad_input(input, -probs / len(target))
